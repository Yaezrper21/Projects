...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function renderSession(currentProfile) {
    if (sessionBanner) {
      if (!currentProfile) {
        sessionBanner.innerHTML = '<strong>Guest Mode</strong><span>Login for purchases.</span>';
      } else {
        sessionBanner.innerHTML = `<strong>${currentProfile.username}</strong><span>${currentProfile.role} via ${currentProfile.authType}.</span>`;
      }
    }
