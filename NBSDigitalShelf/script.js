...
async function openBookModal(bookId, trackView = true, flashMessage = "", flashState = "info") {
  ...
  content.innerHTML = `
    <section class="book-detail">
      ...
      <div class="buy-actions">
        <button class="primary-button full" onclick="purchaseSelectedChapters('${book.id}')">Purchase Selected</button>
      </div>
  ...
  chapterList.querySelectorAll("[data-read-chapter]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      // Inline buy - checkboxes in modal
      const checkboxes = chapterList.querySelectorAll('input[type="checkbox"]');
      checkboxes.forEach(cb => cb.addEventListener('change', updatePurchaseButton));
      updatePurchaseButton();
      
      function updatePurchaseButton() {
        const selected = Array.from(checkboxes).filter(cb => cb.checked).length;
        const purchaseBtn = content.querySelector('.buy-actions button');
        if (purchaseBtn) purchaseBtn.textContent = selected ? `Purchase ${selected} Chapter${selected > 1 ? 's' : ''}` : 'Select Chapters to Buy';
      }
