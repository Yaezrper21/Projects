...
function renderGenreSections(books) {
  ...
  container.innerHTML = Array.from(grouped.keys())
    .sort((a, b) => a.localeCompare(b))
    .map((genre, index) => `
      <div class="genre-section">
        <p class="panel-title">${escapeHtml(genre)}</p>
        <div class="book-row" id="genre-${index}"></div>
      </div>
    `)
    .join("");
  ...
  Array.from(grouped.keys())
    .sort((a, b) => a.localeCompare(b))
    .forEach((genre, index) => fillShelf(`genre-${index}`, grouped.get(genre)));
}
...
async function openBookModal(bookId, trackView = true, flashMessage = "", flashState = "info") {
  ...
  if (!(book.chapters || []).length) {
    ...
    for (const [index, chapter] of (book.chapters || []).entries()) {
      ...
      article.innerHTML = `
        ...
        <div class="chapter-content" data-chapter-content="${chapter.id}" hidden>
          <p>${escapeHtml(chapter.text).replaceAll("\n", "<br>")}</p>
        </div>
      `;
  ...
  chapterList.querySelectorAll("[data-read-chapter]").forEach((button) => {
    button.addEventListener("click", () => {
      const target = chapterList.querySelector(`[data-chapter-content="${button.dataset.readChapter}"]`);
      if (!target) return;
