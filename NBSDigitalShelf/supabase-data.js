...
// Expose for script.js compatibility (replaces auth.js localDB)
window.nbsShelfData = {
  getBooks,
  getBookById,
  getAnnouncements,
...
export async function purchaseMultipleChapters(bookId, chapterIds) {
  if (!Array.isArray(chapterIds) || !chapterIds.length) {
    return { ok: false, message: "No chapters selected." };
  }
  const profile = await getCurrentProfile();
  if (!profile) return { ok: false, message: "Please log in first." };
  ...
  for (const chapterId of chapterIds) {
    const result = await purchaseChapter(bookId, chapterId);
    results.push(result);
  }
  ...
  return {
    ok: true,
    message: `Successfully purchased ${successCount} chapter${successCount === 1 ? '' : 's'}.`
  };
}

export async function purchaseBook(bookId, formData) {
  const profile = await getCurrentProfile();
  if (!profile) return { ok: false, message: "Please log in first." };
  ...
  if (error) return { ok: false, message: "Order failed. " + error.message };

  return { ok: true, message: `Ordered full book: ${book.title}. Check your orders page.` };
}
